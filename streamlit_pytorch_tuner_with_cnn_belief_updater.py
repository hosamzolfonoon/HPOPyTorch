...
""")
    ...
    with col_test:
        test_file = st.file_uploader("Test .npz (optional)", type=["npz"], key="belief_test_npz")
    ...
    if st.button("🚀 Train CNN Belief Updater"):
        if X_train is None or Y_train is None or X_val is None or Y_val is None:
            st.error("Please upload at least Train and Validation .npz files.")
            return

        if X_train.shape[1:] != X_val.shape[1:] or Y_train.shape[1:] != Y_val.shape[1:]:
            st.error(f"Train and validation shapes must match. Train X={X_train.shape}, Val X={X_val.shape}")
            return
        ...
        with tempfile.NamedTemporaryFile(suffix=".pt", delete=False) as tmp:
            ...
            with open(tmp.name, "rb") as f:
                st.download_button(
                    ...
                    f,
                    file_name=f"cnn_belief_updater_{timestamp}.pt",
        ...
        st.download_button(
            ...
            summary_text,
            file_name=f"cnn_belief_updater_summary_{timestamp}.txt",
            mime="text/plain",
        )
...
def main_app():
    ...
    if page == "📄 Introduction & Data Preview":
        ...
        if "df" not in st.session_state:
            ...
            process(df_to_use)
    elif page == "🛰️ CNN Belief Updater":
